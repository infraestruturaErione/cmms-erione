    ...
    private void initializeSubscriptionPlans() {
        if (!subscriptionPlanService.existByCode("FREE")) {
            ...
            subscriptionPlanService.create(SubscriptionPlan.builder()
                    ...
                    .yearlyCostPerUser(0)
                    .features(new HashSet<>(Arrays.asList(PlanFeatures.values()))).build());
        }
        ...
        log.info("Ensuring all subscription plans have all features...");
        subscriptionPlanService.getAll().forEach(plan -> {
            plan.setFeatures(new HashSet<>(Arrays.asList(PlanFeatures.values())));
